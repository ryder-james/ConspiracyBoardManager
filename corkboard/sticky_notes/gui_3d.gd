...
func _unhandled_input(event: InputEvent) -> void:
	...
	for mouse_event in [
		...
		if is_instance_of(event, mouse_event):
			...
			return
	_node_viewport.push_input(event)
...
func _mouse_input_event(_camera: Camera3D, event: InputEvent, 
	...
	if _is_mouse_inside:
		...
		# We need to do these conversions so the event's position is in the 
	...
	if event is InputEventMouseMotion or event is InputEventScreenDrag:
		# If there is not a stored previous position, then we'll assume there 
		# is no relative motion.
		if _last_event_pos2D == null:
			event.relative = Vector2(0, 0)
		# If there is a stored previous position, then we'll calculate the 
		# relative position by subtracting the previous position from the new 
		# position. This will give us the distance the event traveled from 
		# prev_pos.
		else:
...
func rotate_area_to_billboard() -> void:
	...
	var billboard_mode: BaseMaterial3D.BillboardMode = \
			_node_quad.get_surface_override_material(0).billboard_mode
	...
	if billboard_mode > 0:
		...
		var look: Vector3 = camera.to_global(Vector3(0, 0, -100)) \
				- camera.global_transform.origin
		look = _node_area.position + look

		# Y-Billboard: Lock Y rotation, but gives bad results if the camera is tilted.
		if billboard_mode == 2:
			look = Vector3(look.x, 0, look.z)

		_node_area.look_at(look, Vector3.UP)

		# Rotate in the Z axis to compensate camera tilt.
		_node_area.rotate_object_local(Vector3.BACK, camera.rotation.z)
